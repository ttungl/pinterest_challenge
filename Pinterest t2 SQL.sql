...
Movies Database: Find duplicate rentals
...
select mv.title, ct.name
from rental as rt 
   inner join customer as ct on rt.customer_id = ct.id 
   inner join movie as mv on rt.movie_id = mv.id 
group by mv.id, ct.id
having count(*) > 1;
